  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="relative rounded-[2rem] p-8 overflow-hidden bg-white border border-slate-100 shadow-sm group">
        <div className="absolute inset-0 bg-gradient-to-br from-blue-50/80 via-white to-violet-50/80 opacity-100 transition-opacity duration-700"></div>
        
        {/* Animated Orbs */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500 rounded-full blur-[80px] opacity-20 group-hover:opacity-30 transition-opacity duration-1000 translate-x-1/3 -translate-y-1/3 mix-blend-multiply"></div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-6">
          <div className="relative">
            ...
            <div className="w-16 h-16 bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl flex items-center justify-center relative shadow-xl shadow-blue-200 ring-1 ring-white/50">
              <Bell className="w-8 h-8 text-white drop-shadow-md" />
            </div>
          </div>
          <div>
            <h1 className="text-3xl font-black font-display text-blue-950 tracking-tight mb-2 bg-clip-text text-transparent bg-gradient-to-r from-blue-950 to-slate-700">
              Pusat Notifikasi
            </h1>
          ...
          return (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 text-sm font-bold rounded-xl transition-all duration-300 ${
                isActive 
                  ? 'bg-gradient-to-r from-blue-700 to-blue-700 text-white shadow-lg shadow-blue-200/50' 
            ...
            >
              ...
              {tab === 'Belum Dibaca' && (
                <span className={`inline-flex items-center justify-center text-[10px] w-5 h-5 rounded-full font-black ${
                  isActive ? 'bg-white text-blue-700 shadow-sm' : 'bg-rose-500 text-white shadow-sm'
                }`}>
            ...
            {filteredNotifications.map((notif, index) => (
              <div 
                ...
                className={`p-6 flex items-start gap-5 transition-all duration-300 cursor-pointer group relative overflow-hidden ${
                  notif.isUnread ? 'bg-blue-50/40 hover:bg-blue-50/80' : 'bg-white hover:bg-slate-50/80'
                }`}
              >
                {/* Unread indicator line */}
                {notif.isUnread && (
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-blue-600 to-violet-500"></div>
                )}
                ...
                <div className="flex-1 min-w-0 pr-4">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-1.5 gap-2 sm:gap-0">
                    <h3 className={`text-base font-bold truncate ${notif.isUnread ? 'text-blue-950' : 'text-slate-700'}`}>
                      {notif.title}
                    </h3>
