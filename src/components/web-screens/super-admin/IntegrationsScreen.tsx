    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-gradient-to-r from-blue-950 via-slate-800 to-indigo-950 rounded-[3rem] p-10 text-white shadow-2xl shadow-indigo-900/30 relative overflow-hidden flex flex-col md:flex-row justify-between md:items-center gap-8 group">
        ...
        <div className="relative z-10 flex items-center gap-6">
          <div className="w-16 h-16 bg-white/10 backdrop-blur-md rounded-[1.5rem] border border-white/20 flex items-center justify-center shrink-0 shadow-inner group-hover:rotate-12 transition-transform duration-500">
            <Cpu className="w-8 h-8 text-blue-400" strokeWidth="2.5" />
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {connectedApps.map((app) => (
          <div key={app.id} className="bg-white/90 backdrop-blur-xl p-8 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/40 hover:-translate-y-2 hover:shadow-2xl hover:shadow-indigo-900/10 hover:border-blue-100 transition-all duration-500 group flex flex-col relative overflow-hidden">

            <div className="absolute -right-8 -top-8 w-32 h-32 bg-gradient-to-bl from-blue-600/10 to-transparent rounded-full pointer-events-none group-hover:scale-[2] transition-transform duration-700"></div>

            <div className="flex justify-between items-start mb-6 relative z-10">
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 rounded-[1.25rem] bg-gradient-to-br from-blue-50 to-white border border-blue-100/50 shadow-inner flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                  <Webhook className="w-7 h-7 text-blue-700" strokeWidth="2.5" />
                </div>
                <div>
                  <h3 className="font-black text-xl text-blue-950 font-display leading-tight group-hover:text-brand transition-colors">{app.name}</h3>
            ...
            <div className="bg-slate-50 p-5 rounded-[1.5rem] text-sm space-y-4 mb-8 relative z-10 border border-slate-100 group-hover:border-blue-100/50 group-hover:bg-blue-50/30 transition-colors">
            ...
            <div className="flex items-center gap-4 relative z-10 mt-auto">
              ...
              <button
                ...
                className="flex-1 py-4 bg-blue-700 text-white hover:bg-brand rounded-[1.5rem] text-xs font-black transition-all flex items-center justify-center gap-2 shadow-xl hover:shadow-2xl hover:shadow-blue-600/30 hover:-translate-y-0.5"
              >
